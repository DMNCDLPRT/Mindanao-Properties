<section>  
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full p-10">
        @foreach ($properties as $allProperties)
                ...
                <a href="{{ route('property-details', ['id' => $allProperties->id]) }}"
                    ...
                    <div class="shadow p-4 rounded-lg bg-white">
                        <div class="flex justify-center relative rounded-lg overflow-hidden h-52">
                            ...
                            @if ($allProperties->propertyClassification->type->name != 'Land' &&
                            (!empty($allProperties->propertyInfo->bedrooms) || 
                            !empty($allProperties->propertyInfo->bathrooms) || 
                            !empty($allProperties->propertyInfo->car_spaces)))
    
                                <div class="absolute flex justify-center bottom-0 mb-3">
                                    <div class="flex bg-white px-4 py-1 space-x-5 rounded-lg overflow-hidden shadow">
                                        
                                        @if ($allProperties->propertyInfo->bedrooms !== null)
                                            <p class="flex items-center font-medium text-gray-800">
                                                ...
                                                {{ $allProperties->propertyInfo->bedrooms }}
                                            </p>
                                        @endif
                                       
    
                                        @if ( $allProperties->propertyInfo->car_spaces !== null )
                                            <p class="flex items-center font-medium text-gray-800">
                                                <svg class="w-5 h-5 fill-current mr-2" xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 480 512">
                                                    <path
                                                ...
                                                {{ $allProperties->propertyInfo->car_spaces }}
                                            </p>
                                        @endif
    
                                       @if ($allProperties->propertyInfo->bathrooms !== null)
                                        <p class="flex items-center font-medium text-gray-800">
                                                <svg class="w-5 h-5 fill-current mr-2" xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 512 512">
                                                    <path
                                                ...
                                                {{ $allProperties->propertyInfo->bathrooms }}
                                            </p>
                                       @endif
                            ...
                            @if ($allProperties->offerType->name == 'rent')
                                <span
                                    class="absolute top-0 left-0 inline-flex mt-3 ml-3 px-3 py-2 rounded-lg z-10 bg-blue-500 text-sm font-medium text-white select-none">
                                    Rent
                                </span>
                            @else
                                <span
                        ...
                        <div class="mt-4">
                            ...
                            <p class="mt-2 text-sm text-gray-800 line-clamp-1" title="New York, NY 10004, United States">
                                {{$allProperties->propertyLocation->display_name}}
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 grid-rows-2 gap-4 mt-8">
                            <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                                ...
                                <span class="mt-2 xl:mt-0">
                                    ...
                                    {{ $allProperties->propertyClassification->type->name }}
                                </span>
                            </p>
                            <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                                <svg class="inline-block w-5 h-5 xl:w-4 xl:h-4 mr-3 fill-current text-gray-800"
                                ...
                                <span class="mt-2 xl:mt-0">
                                    {{ $allProperties->propertyInfo->fully_furnished != 'Yes' ? 
                                       ($allProperties->propertyInfo->fully_furnished == 'Semi' ? 'Semi-Furnished' : 'Unfurnished') : 
                                       'Furnished' }}
                                </span>
                            ...
                            <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                                ...
                                <span class="mt-2 xl:mt-0">
                                    {{ $allProperties->propertyInfo->sqm_range }} sq. ft.
                                </span>
                            </p>
                            <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                                <svg class="inline-block w-5 h-5 xl:w-4 xl:h-4 mr-3 fill-current text-gray-800"
                        ...
                        <div class="grid grid-cols-2 mt-8">
                            <div class="flex items-center">
                                <div class="relative">
                                    <div class="rounded-full w-6 h-6 md:w-8 md:h-8 bg-gray-200">
                                        ...
                                        <img src="{{ asset($allProperties->user->profile_photo_url) }}" class="rounded-full"
                                            alt="profile">
                            ...
                            <div class="flex justify-end">
                                <p
                                    ...
                                    <span class="text-lg">{{ $allProperties->propertyInfo->price_php }}</span>/m
                                </p>
                            </div>
                        </div>
